...
namespace CBF {
	struct ObjectNamespace {
		...
		*/
		template<class T> boost::shared_ptr<T> get(const std::string &key, bool throw_if_fails = false) {
			try {
				ObjectPtr p = m_Map[key];
				boost::shared_ptr<T> ret = boost::dynamic_pointer_cast<T>(p);
				if (ret.get() == 0) CBF_THROW_RUNTIME_ERROR("cast failed");
			} catch (...) {
				if (throw_if_fails) throw;
