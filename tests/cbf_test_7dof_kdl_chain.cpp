...
int main() {
	...
	SensorTransformPtr kdstPtrcopy = kdlst;
	cout << "kdstPtrcopy->resourcedim=" << kdlst->resource_dim() << endl;

	std::vector<SensorTransformPtr> sensorTransforms;
	sensorTransforms.push_back(kdlst);
	cout << "sensorTransforms[0]->resourcedim=" << sensorTransforms[0]->resource_dim() << endl;

	std::vector<SensorTransformPtr> sensorTransformsCopy;
	sensorTransformsCopy =  sensorTransforms;
	cout << "sensorTransformsCopy[0]->resourcedim=" << sensorTransformsCopy[0]->resource_dim() << endl;

//	std::vector<SensorTransformPtr> sensorTransforms;
//	sensorTransforms.push_back(
//		   KDLChainPositionSensorTransformPtr(
//				   new KDLChainPositionSensorTransform(lwrchain)));
//	if (!only3DControl) {
//	   sensorTransforms.push_back(
//			   KDLChainAxisAngleSensorTransformPtr(
//					   new KDLChainAxisAngleSensorTransform(lwrchain)));
//	}
	cout << sensorTransforms[0]->resource_dim() << endl;
	CompositeSensorTransformPtr sensortransform(new CompositeSensorTransform(sensorTransforms));

	cout << "EEEEEEEEEEEEEEEENNNNNNNNNNNNNNNNNNNNNNNNNNNDDDDDDDDDDDDDDDDDDDD" << endl;
	return 0;
